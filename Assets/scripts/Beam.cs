...
public class Beam : MonoBehaviour {
		...
		{  
			...
			{  
				...
				if(i==0)  
				{  
				if (Physics.Raycast (ray.origin, ray.direction, out hit, 100) && hit.transform.tag == "Player") {
					hit.transform.SendMessage ("HitByRay");
					if (Physics.Raycast (ray.origin, ray.direction, out hit, 100) && hit.transform.tag == "Enemy") {
						hit.transform.SendMessage ("HitByRay2");
					}
				}
					//Check if the ray has hit something  
					if(Physics.Raycast(ray.origin,ray.direction, out hit, 100)&&hit.transform.tag=="Laser")//cast the ray 100 units at the specified direction  
					{ 
